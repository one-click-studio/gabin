...
import ButtonContainerUi from '@src/renderer/src/components/basics/ButtonContainerUi.vue'

import { onAlphaNumPress, onSpacePress } from '@src/renderer/src/components/utils/KeyPress.vue'

import PlayCircleIcon from '@src/renderer/src/components/icons/PlayCircleIcon.vue'
import PauseCircleIcon from '@src/renderer/src/components/icons/PauseCircleIcon.vue'
import StopIcon from '@src/renderer/src/components/icons/StopIcon.vue'
import CamIcon from '@src/renderer/src/components/icons/CamIcon.vue'

import type { Shoot, AvailableMicsMap, ObsAssetId, ObsSource } from '@src/types/protocol'
...
const init = () => {
    ...
    handle.handleNewShot(async (_, shot) => {
        setCurrentShot(shot.data)
    })
    handle.handleAutocam(async (_, ac) => {
        autocam.value = ac.data
    })
    handle.handleAvailableMics(async (_, am) => {
        availableMics.value = am.data
    })
...
const toggleAutocam = () => {
    invoke.toggleAutocam(!autocam.value)
}

const toggleMicAvailability = (mic: string) => {
    invoke.toggleAvailableMic(mic)
}

const triggerShot = (i: number, j: number) => {
    invoke.triggerShot(klona(shots.value[i][j]))
}
